 ...
 *  Brep.ts
 * 
 *    Provides a high-level API for creating and manipulating BREP geometries based on OpenCascade.
 *    
 *    Basic functionality:
 *  
 *      - create 2D/3D BREP shapes:
 * 
 *          brep = new Brep();
 *          myBox = brep.Box(100,50,20);
 *          myBoxBetween = brep.BoxBetween([0,0,0], [100,50,20]);
 *          mySphere = brep.Sphere(30);
 *          myRect = brep.Rect(10,20);
 * 
 *          
 *      - 2D Sketches
 *          
 *          brep = new Brep();
 *          mySketch = brep.sketch()
 *                      .lineTo(50,100)
 *                      .lineTo(100,0)
 *                      .close();
 *        
 *      - Scene management
 *          All Shapes created are automatically added to Scene (see Obj class in Obj.ts). 
 *          You can manage the scene, its Obj instances and their properties:
 * 
 *          brep = new Brep();
 *          myBox = brep.Box(50).color('red');
 *          brep.layer('spheres').color('green');
 *          mySphere = brep.Sphere(); // automatically added to active layer and colored green
 *          
 *          console.log(brep.scene.toGraph()); // scene graph
 * 
 *    NOTES:
 * 
 *    - Shortcuts in archiyou scripts: To simplify scripts on our platform some functions of brep are 
 *        added to the execution scope: 
 *          so new Brep() is done automatically
 *          and methods like brep.Box() are accessable as box()
 *        
 *        If you see shortcuts like this you need to Runner to automatically handle create shortcuts
 * 
 *    - [TODO] BREP and MESH: BREP offers great control, but can be slow for big models. Next to the Brep class we are creating a mesh one.
 *  
 */

import { Vector, Point, Obj, Shape, Vertex, Edge, Wire, Face, Shell, 
  Solid, Bbox, OBbox, ShapeCollection, VertexCollection, Sketch } from './internal'

import type { PointLike, SelectionString,PointLikeSequence, MakeWireInput,
        MakeFaceInput, Axis, MakeShellInput, MakeSolidInput,
        AnyShapeOrCollection, AnyShapeOrCollectionOrSelectionString, ModelUnits } from './internal' // types

import { SketchPlaneName } from './internal' // Sketch
...
export class Brep
{
  ...
  constructor()
  {
    ...
    //Vector.prototype._brep = this;
    //Point.prototype._brep = this;
    Bbox.prototype._brep = this;
    Obj.prototype._brep = this; 
    Shape.prototype._brep = this;
    ShapeCollection.prototype._brep = this;
    Sketch.prototype._brep = this;

    Annotator.prototype._brep = this;
  ...
  Vector(p:PointLike, ...args):Vector
  {
    ...
    console.geom(`brep::Vector: Created a Vector with coordinates ${v.x},${v.y},${v.z}`);
    return v;
  }
  ...
  Vertex(p:PointLike, ...args):Vertex
  {
    ...
    console.geom(`brep::Vertex: Created a Vertex at ${v.x},${v.y},${v.z}`);
    return v;
  }
  ...
  Edge(start:PointLike, end:PointLike):Edge
  {
    ...
    console.geom(`brep::Edge: Created a Edge with length ${edge.length()}`);
  ...
  Line(start:PointLike, end:PointLike):Edge
  {
    ...
    console.geom(`brep::Line: Created a Line Edge with length ${edge.length()}`);
    return edge;
  }
  ...
  Arc(start:PointLike, mid:PointLike, end:PointLike):Edge
  {
    ...
    console.geom(`brep::Arc: Created a Arc Edge with start [${startVertex.x},${startVertex.y},${startVertex.z}], mid [${midVertex.x},${midVertex.y},${midVertex.z}] and end [${endVertex.x},${endVertex.y},${endVertex.y}]`);
    return edge;
  }
  ...
  Spline(points:PointLikeSequence, ...args):Edge
  {
    ...
    console.geom(`brep::Spline: Created a Spline Edge through ${vertices.length} points with length "${edge.length()}"`);
    return edge;
  }
  ...
  Wire(entities:MakeWireInput, ...args):Wire
  {
    ...
    console.geom(`brep::Wire: Created a Wire with ${wire.edges().length} Edges and total length ${wire.length()}`);
  ...
  Spiral(firstRadius:number, secondRadius:number, angle:number, lefthand:boolean)
  {
    ...
    console.geom(`brep::Wire: Created a Spiral`);
  ...
  Helix(radius?:number, height?:number, angle?:number, pivot?:PointLike, direction?:PointLike, lefthand?:boolean, coneSemiAngle?:number)
  {
    ...
    console.geom(`brep::Wire: Created a Helix`);
  ...
  Face(entities:MakeFaceInput, ...args):Face|Shell
  {
    ...
    */
    ...
    console.geom(`brep::Face: Created a ${faceOrShell.type()} with ${faceOrShell.vertices().length} vertices`);
    return faceOrShell;
  }
  ...
  Plane(width?:number, depth?:number, normal?:PointLike, position?:PointLike):Face
  {
    ...
    console.geom(`brep::Plane: Created a Plane Face with area ${plane.area()}`);
    return plane;
  }
  ...
  RectBetween(from:PointLike, to:PointLike):Face
  {
    ...
    console.geom(`brep::Rect: Created a Rect Face with size [${rect.bbox().width()},${rect.bbox().depth()}]`);
    return rect;
  }
  ...
  BasePlane(axis?:string, size?:number):Face
  {
    ...
    plane.name(this.getNextObjName('Plane')); // auto name
    console.geom(`brep::Plane: Created a Plane Face with size [${plane.bbox().width()},${plane.bbox().depth()}]`);
    return plane;
  }
  ...
  Circle(radius?:number):Face|Shell
  {
    ...
    console.geom(`brep::Circle: Created a Circular Face with radius ${radius}`);
    return circleFace;
  }
  ...
  Shell(entities: MakeShellInput, ...args):Shell
  {
    ...
    shell.addToScene();
    console.geom(`brep::Shell: Created a Shell with ${shell.faces().length} Faces`);
    return shell;
  }
  ...
  Solid(shells:MakeSolidInput, ...args):Solid
  {
    ...
    solid.addToScene();
    console.geom(`brep::Shell: Created a Solid with ${solid.shells().length} Shells and ${solid.faces().length} Faces`);
    return solid;
  }
  ...
  Box(width?:number, depth?:number, height?:number, position?:PointLike):Solid
  {
    ...
    console.geom(`brep::Box: Created a Box Solid with size [${width}, ${depth||width}, ${height||width}] at [${box.center().x},${box.center().y},${box.center().z}]`);
    return box;
  }
  ...
  BoxBetween(from:PointLike, to:PointLike):Solid
  {
    ...
    if (!box)
    {
      throw Error(`brep::Box: Failed to create Box between points ${from} and ${to}. Check if Point span a 3D space!`);
    }
    box.addToScene();
    box.name(this.getNextObjName('Box')); // auto name
    console.geom(`brep::Box: Created a Box Solid with size [${box.bbox().width()}, ${box.bbox().depth()}, ${box.bbox().height()}] at [${box.center().x},${box.center().y},${box.center().z}]`);
    return box;
  }
  ...
  Sphere(radius?:number, position?:PointLike): Solid
  {
    ...
    console.geom(`brep::Sphere: Created a Sphere Solid with radius ${radius} at [${sphere.center().x},${sphere.center().y},${sphere.center().z}]`);
    return sphere;
  }
  ...
  Cone( bottomRadius?:number, topRadius?:number, height?:number, position?:PointLike):Solid
  {
    ...
    console.geom(`brep::Cone: Created a Cone Solid with radii [${bottomRadius},${topRadius}] at [${cone.center().x},${cone.center().y},${cone.center().z}]`);
    ...
    Cylinder(radius?:number, height?:number, position?:PointLike, angle?:number):Solid
  {
    ...
    console.geom(`brep::Cylinder: Created a Cylinder Solid with radius ${radius} at [${cylinder.center().x},${cylinder.center().y},${cylinder.center().z}]`);
  ...
  */
  ...
  addToActiveLayer(obj:Obj):this
  {
    ...
    if(activeLayer && !activeLayer.has(obj))
    {
      ...
      console.warn(`brep::addToActiveLayer: Obj is already in the scene!`);
    }
    return this;
  }
  ...
  */
  layer(name?:string):Obj
  {
      ...
      if (!layerObj)
      {
        ...
        console.geom(`brep::layer:: Created new layer "${name}"`);
      }
      else 
      {
  ...
  setActiveLayer(layerObj:Obj):Obj
  {
      ...
      console.info(`brep::setActiveLayer: Set "${this.activeLayer.name()}" as active! All object newly created will be on that layer`);
  ...
  getLayerShapes(layerName:string=null):ShapeCollection
  {
    ...
    if(!layer)
    {
      throw Error(`brep::getLayerShapes: Cannot get layer "${layerName}": Check if it exists!`);
    }
  ...
  deleteLayer(layerName:string=null):boolean
  {
    ...
    {
      throw Error(`brep::deleteLayer: No layer found with name "${layerName}`);
    }
  ...
  */
  layerGroup(name:string, inObjName?:string):Obj
  {
      ...
      console.info(`brep::groupLayers: Started grouping layers under name "${name}"`);
  ...
  _captureShapesEnd():ShapeCollection
  {
    ...
    { 
      throw new Error(`brep::_captureShapesEnd(): Please use '_captureShapes()' to start capture first!`) 
    }
