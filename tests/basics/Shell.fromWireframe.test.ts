import { Shell, Brep, OcLoader } from '../../src/internal' // import only from internal, otherwise we get circular import problems
...
test("Shell.fromWireframe plane", () => 
{
    ...
    const edges = brep.Plane(100,50).edges().moved(150);
...
test("Shell.fromWireframe box", () => 
    {
        // Box wireframe
    const edges2 = brep.Box(60,50,20).move(300).edges();
...
test("Shell.fromWireframe fragmented box", () => 
{
    ...
    const edges3 = brep.Box(60,50,20).move(300).select('E[0-3] and E[7-11]');
